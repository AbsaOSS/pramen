...
| `save.mode`                               | (experimental) Save mode to use when writing partitions. Supported: `overwrite` (default), `append`.                                                                                                                                                                |
...
| `passive`         | No           | If `true`, failing dependent job will generate an error, but the dependent job is still going to run and their range dependencies checked. If they fail, the job is going to be skipped for the information date. This is useful for operations that need to happen as soon as dependencies are met, but there is no certainty regarding the exact date. |
