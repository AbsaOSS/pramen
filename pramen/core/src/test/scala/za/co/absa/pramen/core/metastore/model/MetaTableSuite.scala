...
class MetaTableSuite extends AnyWordSpec {
  ...
  "fromConfigSingleEntity()" should {
    ...
    "load a metatable definition with hive table defined" in {
      ...
      val metaTable = MetaTable.fromConfigSingleEntity(conf, appConf, "INFO_DATE", "dd-MM-yyyy", defaultPartitionByInfoDate = true, LocalDate.parse("2020-01-31"), 1, defaultHiveConfig, defaultPreferAddPartition = true, "batchid")
    ...
    "throw an exception when mandatory the option is missing" in {
      ...
      val ex = intercept[IllegalArgumentException] {
        MetaTable.fromConfigSingleEntity(conf, conf, "", "", defaultPartitionByInfoDate = true, LocalDate.parse("2020-01-31"), 0, defaultHiveConfig, defaultPreferAddPartition = true, "batchid")
      }
