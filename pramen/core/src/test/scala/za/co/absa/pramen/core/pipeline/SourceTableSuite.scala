...
class SourceTableSuite extends AnyWordSpec {
  "fromConfig" should {
    ...
    "throw an exception in case of an incorrect query type" in {
      ...
      assert(ex.getMessage.contains("No options are specified for the 'input' query. Usually, it is one of: 'input.sql', 'input.path', 'input.table', 'input.db.table' at source.tables[1]."))
    }
