...
trait ExecutionsTable {
  ...
  class ExecutionsRecords(tag: Tag) extends Table[Execution](tag, "executions") {
    ...
    def * = (pipelineId, pipelineName, environmentName, batchId, sparkApplicationId,
      computeEngineId, tenant, country, runDateFrom, runDateTo, startedAt, finishedAt, numberOfExecutorsMin,
      numberOfExecutorsMax, executorType, status, isRerun, attemptNumber, numberOfAttempts, failureReason,
      additionalOptions) <> (Execution.tupled, Execution.unapply)
