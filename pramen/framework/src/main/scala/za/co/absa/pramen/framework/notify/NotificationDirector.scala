...
object NotificationDirector {

  /**
    * Apply the builder steps in order to create a formatted notification.
    */
  def build(notificationBuilder: NotificationBuilder,
    ...
    notificationBuilder.addAppName(notification.ingestionName)
    notificationBuilder.addEnvironmentName(notification.environmentName)
    notificationBuilder.addAppDuration(notification.syncStarted, notification.syncFinished)
    ...
    notification.schemaDifferences.foreach(notificationBuilder.addSchemaDifferences)
