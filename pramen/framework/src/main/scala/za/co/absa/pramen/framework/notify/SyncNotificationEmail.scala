...
class SyncNotificationEmail(notification: Notification)
                           (implicit conf: Config) extends Sendable {
  private val log = LoggerFactory.getLogger(this.getClass)

  private val notificationBuilder = new NotificationBuilderHtml

  NotificationDirector.build(notificationBuilder, notification)
  ...
  override def getTo: String = {
    if (conf.hasPath(Keys.MAIL_FAILURES_TO)) {
      if (notification.exception.isDefined || notification.tasksCompleted.exists(_.failureReason.isDefined)) {
