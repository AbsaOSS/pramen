...
package za.co.absa.pramen.framework.job.v2.operation
...
import za.co.absa.pramen.framework.job.v2.OperationType.{Ingestion, PythonTransformation, Sink, Transformation}
import za.co.absa.pramen.framework.job.v2.job.PythonTransformationJob._
import za.co.absa.pramen.framework.job.v2.job._
import za.co.absa.pramen.framework.job.{SinkTable, SourceTable}
...
class OperationSplitter(conf: Config,
  ...
  def createPythonTransformation(operationDef: OperationDef,
                                 ...
                                 outputTable: String)(implicit spark: SparkSession): Seq[Job] = {
    val outputMetaTable = metastore.getTableDef(outputTable)
    val syncWatcherPyLocation = conf.getString(SYNCWATCHER_PY_LOCATION_KEY)
    val syncWatcherPyExecutable = conf.getString(SYNCWATCHER_PY_EXECUTABLE_KEY)
    val cmdLineTemplate = conf.getString(SYNCWATCHER_PY_CMD_LINE_TEMPLATE_KEY)
