...
class TransformerMock(conf: Config) extends Transformer {
  override def validate(metastore: MetastoreReader,
                        infoDate: LocalDate,
                        options: Map[String, String]): Reason = Reason.Ready

  override def run(metastore: MetastoreReader, infoDate: LocalDate, options: Map[String, String]): DataFrame = {
    val df = metastore.getTable("table1", None, None)
    df
      .withColumn("c", lit(options("opt1")))
      .withColumn("d", lit(Date.valueOf(infoDate)))
  }
}
