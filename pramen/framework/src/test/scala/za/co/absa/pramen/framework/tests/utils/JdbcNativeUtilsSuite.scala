...
class JdbcNativeUtilsSuite extends WordSpec with RelationalDbFixture with SparkTestBase {
  ...
  "getConnection()" should {
    "select working connection when provided a connection pool" in {
      val jdbcConfig = JdbcConfig(driver, Some("bogus_url"), "bogus_url2" :: url :: Nil, None, user, password, Map.empty[String, String])
  ...
  "getJdbcNativeRecordCount()" should {
    val conf = JdbcConfig(driver, Some(url), Nil, None, user, password, Map.empty[String, String])
